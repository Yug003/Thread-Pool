[README.md](https://github.com/user-attachments/files/30587060/README.md)
...
```cpp
try {
    for (size_t i = 0; i < threads; ++i) {# A Correct, Thread-Safe, and Exception-Safe C++17 Thread Pool
...
* **Header-Only**: Drop [thread_pool.hpp](thread_pool.hpp) straight into your project.
...
`main.cpp` is a 7-case verification suite covering both correctness and concurrency edge cases:

| # | Test | What it verifies |
|---|------|-------------------|
| 1 | `test_basic_execution` | Basic task submission and return values via `std::future` |
| 2 | `test_move_only_arguments` | Move-only arguments (`std::unique_ptr`) are forwarded correctly |
| 3 | `test_exception_propagation` | Exceptions thrown inside a task propagate through `future.get()` |
| 4 | `test_graceful_shutdown` | Destructor blocks until all queued tasks finish |
| 5 | `test_immediate_shutdown` | Non-graceful shutdown discards pending tasks and rejects new ones |
| 6 | `test_concurrency_stress` | 8,000 tasks submitted concurrently from 8 submitter threads complete exactly once |
| 7 | `test_constructor_exception_safety` | Requesting an unreasonable thread count cleans up without `std::terminate()` |
...
`benchmark.cpp` measures task throughput against two baselines on a CPU-bound workload (counting primes via trial division), so the numbers reflect real scheduling overhead rather than I/O wait:

* **Single-threaded**: tasks run sequentially, no threading at all.
* **Naive thread-per-task**: a new `std::thread` is spawned and joined for every task.
* **ThreadPool**: tasks are submitted to this pool via `enqueue()`.

```bash
# Compiling
g++ -std=c++17 -O2 -pthread benchmark.cpp -o benchmark

# Running (args: number of tasks, work size per task)
./benchmark 2000 20000
```

Results scale with the number of physical cores available (`std::thread::hardware_concurrency()`), and will show the most improvement over the naive thread-per-task baseline, since that approach pays full OS thread-creation cost on every single task.

**Measured results** (2,000 tasks, each counting primes below 20,000, 8-core machine):

| Approach                | Time (ms) |
|--------------------------|-----------|
| Single-threaded          | 1894      |
| Naive thread-per-task    | 495       |
| ThreadPool               | 440       |

**~4.3x speedup over single-threaded execution**, and a further edge over naive thread-per-task by avoiding repeated OS thread-creation overhead.
        workers.emplace_back(&ThreadPool::worker_loop, this);
    }
} catch (...) {
    {
